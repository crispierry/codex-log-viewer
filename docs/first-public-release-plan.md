...
| Native UI automation | Complete for the release-critical workflow. Deeper VoiceOver QA and selection-navigation shortcuts remain post-v0.1 polish. | `scripts/smoke-native-ui.mjs`, accessibility identifier checks in `apps/macos/Sources/CodexLogViewerMac/RootView.swift`, selected-project export/session smoke coverage, and CI `macos-app`. |
